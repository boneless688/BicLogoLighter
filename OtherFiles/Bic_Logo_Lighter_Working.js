...
var menuArray = [];
for (n = 0; n < lima.length; n++) {
	menuArray.push(capitalizeSpaces(lima[n].fullName));
}
...
var regularsRepName = customerInfo_outer.add(
	"statictext",
	undefined,
	"Rep name"
);
...
if (
     ...
     if (regulars.selection > 0) {
          for (i = 0; i < menuArray.length - 1; i++) {
			if (regulars.selection.toString() === menuArray[i]) {
				rep.name = capitalizeSpaces(lima[i].fullName);
				rep.email = lima[i].email;
				rep.company = capitalizeSpaces(lima[i].company);
				rep.nationality = lima[i].nationality;
				rep.vip = lima[i].vip;
				}
			}
	} else {
     ...
     repName.contents = rep.name;
			...
			if (rep.vip === "y") {
				var vipBoxNew = app.activeDocument.layers
					.getByName("Masque")
					.groupItems.getByName("VIP");
				vipBoxNew.hidden = false;
			}
